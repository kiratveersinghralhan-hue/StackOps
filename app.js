(() => {
  ...
  function bindTeamReportButtons(){
    ...
    qsa(".report-trigger").forEach(btn => btn.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  function bindAuthFlow(){
    ...
    if(riotBtn){
      riotBtn.addEventListener("click", () => {
        alert("Riot sign-in needs Riot OAuth approval and backend credentials. Keep this as coming soon until configured.");
      });
    }
  }
  ...
  function bindLogout(){
    ...
    if(btn) btn.addEventListener("click", async () => {
      if(state.supabase){
        try{ await state.supabase.auth.signOut(); }catch(err){ console.error(err); }
      }
      setLoggedIn(false);
    });
  }
  ...
  function initIntro(){
    ...
    document.addEventListener("DOMContentLoaded", () => setTimeout(hideIntro, 1800), { once:true });
  ...
  async function init(){
    ...
    await syncSession();
    if(state.supabase){
      state.supabase.auth.onAuthStateChange(async () => { await syncSession(); });
    }
  }
