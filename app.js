...
/* === StackOps trust carousel and seller review system === */
(function(){
  let manualPaymentContext = null;
  const UPI_ID = (window.STACKOPS_CONFIG && window.STACKOPS_CONFIG.MANUAL_UPI_ID) || 'ralhanx@ptaxis';
  const UPI_QR = (window.STACKOPS_CONFIG && window.STACKOPS_CONFIG.MANUAL_UPI_QR_URL) || 'upi.jpeg';

  function safeToast(msg){ if(typeof toast === 'function') toast(msg); else alert(msg); }
  function moneySafe(n){ return typeof money === 'function' ? money(n) : `₹${Number(n||0).toLocaleString('en-IN')}`; }
  function esc(x){ return typeof escapeHtml === 'function' ? escapeHtml(x || '') : String(x || '').replace(/[&<>"']/g, m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#039;"}[m])); }
  function currentUserId(){ return session?.user?.id || null; }

  async function signedOrPublicUrl(bucket, path){
    if(!path || !sb) return '';
    try{
      ...
      try { return sb.storage.from(bucket).getPublicUrl(path).data.publicUrl || ''; } catch(_) { return ''; }
    }
  }

  window.openManualPayment = function(ctx){
    if(typeof needLogin === 'function' && needLogin()) return;
    manualPaymentContext = ctx || {};
    const modal = document.getElementById('manualPaymentModal');
    if(!modal) return safeToast('Payment modal missing');
    document.getElementById('manualPayTitle').textContent = manualPaymentContext.name || 'Complete Payment';
    document.getElementById('manualPayAmount').textContent = moneySafe(manualPaymentContext.amount || 0);
    document.getElementById('manualPayUPI').textContent = UPI_ID;
    document.getElementById('manualPayQR').src = UPI_QR;
    document.getElementById('manualPayUTR').value = '';
    document.getElementById('manualPayProof').value = '';
    modal.classList.add('show');
  };

  window.buy = function(name, amount, type='service'){
    openManualPayment({ name, amount, item_type:type });
  };
  window.buyPlan = function(planKey){
    const plan = (demo?.plans || []).find(p=>p.key===planKey);
    if(!plan) return safeToast('Plan not found');
    if(Number(plan.price_inr||0) <= 0) return safeToast('Free plan is already active');
    openManualPayment({ name:`StackOps ${plan.name} Plan`, amount:plan.price_inr, item_type:'plan', plan_key:plan.key });
  };

  async function submitManualPaymentProof(){
    if(!sb) return safeToast('Supabase not connected');
    const user = session?.user || (await sb.auth.getUser()).data.user;
    if(!user) return safeToast('Login required');
    const utr = document.getElementById('manualPayUTR')?.value?.trim();
    const file = document.getElementById('manualPayProof')?.files?.[0];
    const allowPublic = !!document.getElementById('manualPayPublicFeedback')?.checked;
    if(!utr) return safeToast('Enter UTR / Reference number');
    if(!file) return safeToast('Upload payment screenshot with UTR visible');
    const amount = Number(manualPaymentContext?.amount || 0);
    ...
    if(uploadError) return safeToast('Screenshot upload failed: ' + (uploadError.message || uploadError));
    const commission = typeof window.commission === 'function' ? window.commission(amount) : Math.ceil(amount * 0.15);
    const order = {
      buyer_id:user.id,
      seller_id:manualPaymentContext?.seller_id || null,
      item_name:manualPaymentContext?.name || 'StackOps purchase',
      item_type:manualPaymentContext?.item_type || 'service',
      plan_key:manualPaymentContext?.plan_key || null,
      amount_inr:amount,
      commission_inr:commission,
      seller_earning_inr:Math.max(0, amount - commission),
      utr,
      proof_url:path,
      status:'pending',
      is_feedback_public:allowPublic,
      feedback_text:'Payment completed and submitted for StackOps verification.'
    };
    const { error } = await sb.from('manual_orders').insert(order);
    if(error) return safeToast('Could not submit payment: ' + error.message);
    document.getElementById('manualPaymentModal')?.classList.remove('show');
    safeToast('Payment submitted. It will reflect in your account within 24–48 hours after verification.');
    loadTrustCarousel();
    if(typeof renderAdmin === 'function' && typeof isAdmin === 'function' && isAdmin()) renderAdmin();
  }

  async function loadTrustCarousel(){
    const el = document.getElementById('paymentProofCarousel');
    if(!el) return;
    if(!sb){ el.innerHTML = '<div class="empty-state">Payment feedback will appear after real admin-approved orders.</div>'; return; }
    try{
      const { data, error } = await sb.from('manual_orders')
        .select('id,amount_inr,proof_url,utr,created_at,feedback_text')
        .eq('status','approved')
        .eq('is_feedback_public',true)
        .order('created_at',{ascending:false})
        .limit(10);
      if(error) throw error;
      if(!data || !data.length){ el.innerHTML = '<div class="empty-state">No public payment feedback yet. Approved proofs will appear here.</div>'; return; }
      const cards = await Promise.all(data.map(async(o)=>{
        const img = await signedOrPublicUrl('payment-proofs', o.proof_url);
        const utr = o.utr ? String(o.utr).slice(0,4)+'••••'+String(o.utr).slice(-3) : 'verified';
        return `<article class="trust-card"><img src="${img}" alt="Payment proof"><div class="trust-meta"><div><b>${moneySafe(o.amount_inr)}</b><small>Verified payment</small></div><span class="chip">UTR ${esc(utr)}</span></div><p class="muted">${esc(o.feedback_text || 'Payment completed and approved by StackOps admin.')}</p></article>`;
      }));
      el.innerHTML = cards.join('');
    }catch(e){ el.innerHTML = '<div class="empty-state">Payment feedback is currently unavailable.</div>'; }
  }

  async function loadSellerReviews(){
    const el = document.getElementById('sellerReviewWall');
    if(!el) return;
    if(!sb){ el.innerHTML = '<div class="empty-state">Reviews will appear after real buyers submit feedback.</div>'; return; }
    try{
      const { data, error } = await sb.from('seller_reviews')
        .select('id,rating,review_text,seller_name,item_name,created_at')
        .eq('status','approved')
        .order('created_at',{ascending:false})
        .limit(12);
      if(error) throw error;
      if(!data || !data.length){ el.innerHTML = '<div class="empty-state">No approved reviews yet.</div>'; return; }
      el.innerHTML = data.map(r=>`<article class="review-card"><div class="stars">${'★'.repeat(Math.max(1,Math.min(5,Number(r.rating||5))))}${'☆'.repeat(5-Math.max(1,Math.min(5,Number(r.rating||5))))}</div><h3>${esc(r.seller_name || r.item_name || 'Seller')}</h3><p>${esc(r.review_text || '')}</p><small>${new Date(r.created_at || Date.now()).toLocaleDateString()}</small></article>`).join('');
    }catch(e){ el.innerHTML = '<div class="empty-state">Reviews are currently unavailable.</div>'; }
  }
  ...
  async function submitSellerReview(){
    ...
    const user = session?.user || (await sb.auth.getUser()).data.user;
    if(!user) return safeToast('Login required');
    const sellerName = document.getElementById('reviewSellerName')?.value?.trim();
    const rating = Number(document.getElementById('reviewRating')?.value || 5);
    const text = document.getElementById('reviewText')?.value?.trim();
    if(!sellerName || !text) return safeToast('Enter seller/service name and review');
    const { error } = await sb.from('seller_reviews').insert({ reviewer_id:user.id, seller_name:sellerName, rating, review_text:text, status:'pending' });
    if(error) return safeToast('Review failed: ' + error.message);
    document.getElementById('reviewModal')?.classList.remove('show');
    safeToast('Review submitted. It will appear after admin approval.');
  }

  async function loadAdminReviewQueues(){
    if(!(typeof isAdmin === 'function' && isAdmin()) || !sb) return;
    const grid = document.querySelector('.admin-grid');
    if(!grid) return;
    if(!document.getElementById('adminReviewQueues')){
      grid.insertAdjacentHTML('beforeend', `<section class="panel" id="adminReviewQueues"><div class="panel-head"><h2>Trust Reviews</h2><span class="chip">Admin</span></div><div id="adminReviewsList" class="admin-list"></div></section>`);
    }
    const { data } = await sb.from('seller_reviews').select('*').order('created_at',{ascending:false}).limit(30).then(undefined,()=>({data:[]}));
    const list = document.getElementById('adminReviewsList');
    if(list) list.innerHTML = (data||[]).map(r=>`<div class="user-row"><b>${esc(r.seller_name || 'Review')}</b><small>${'★'.repeat(Number(r.rating||5))} · ${esc(r.status || 'pending')} · ${esc(r.review_text || '')}</small><button class="mini" onclick="approveReview('${r.id}')">Approve</button><button class="mini danger" onclick="rejectReview('${r.id}')">Reject</button></div>`).join('') || 'No reviews yet';
  }
  window.approveReview = async id => { await sb.from('seller_reviews').update({status:'approved'}).eq('id',id); safeToast('Review approved'); loadSellerReviews(); loadAdminReviewQueues(); };
  window.rejectReview = async id => { await sb.from('seller_reviews').update({status:'rejected'}).eq('id',id); safeToast('Review rejected'); loadAdminReviewQueues(); };

  const oldRenderAdmin = typeof renderAdmin === 'function' ? renderAdmin : null;
  if(oldRenderAdmin){ renderAdmin = async function(){ await oldRenderAdmin(); await loadAdminReviewQueues(); }; }
  const oldRenderAll = typeof renderAll === 'function' ? renderAll : null;
  if(oldRenderAll){ renderAll = function(){ oldRenderAll(); loadTrustCarousel(); loadSellerReviews(); }; }

  document.addEventListener('DOMContentLoaded',()=>{
    document.getElementById('closeManualPay')?.addEventListener('click',()=>document.getElementById('manualPaymentModal')?.classList.remove('show'));
    document.getElementById('copyManualUPI')?.addEventListener('click',async()=>{ try{ await navigator.clipboard.writeText(UPI_ID); }catch(e){} safeToast('UPI ID copied'); });
    document.getElementById('submitManualPayment')?.addEventListener('click',submitManualPaymentProof);
    document.getElementById('openReviewBtn')?.addEventListener('click',()=>document.getElementById('reviewModal')?.classList.add('show'));
    document.getElementById('closeReviewModal')?.addEventListener('click',()=>document.getElementById('reviewModal')?.classList.remove('show'));
    document.getElementById('submitSellerReview')?.addEventListener('click',submitSellerReview);
    setTimeout(()=>{ loadTrustCarousel(); loadSellerReviews(); loadAdminReviewQueues(); }, 1200);
  });
})();

/* === StackOps seller dashboard: ecommerce orders, earnings, payouts + path fixes === */
(function(){
  const q = (s) => document.querySelector(s);
  const cfg = window.STACKOPS_CONFIG || {};
  if(cfg.MANUAL_UPI_QR_URL && String(cfg.MANUAL_UPI_QR_URL).includes('assets/')) cfg.MANUAL_UPI_QR_URL = 'upi.jpeg';
  function notify(msg){ try{ toast(msg); }catch{ alert(msg); } }
  ...
  function sellerActive(){ return !!(me?.is_seller || me?.seller_status === 'approved' || localStorage.stackopsSellerApproved === '1'); }
  function statusPill(s){ const st=(s||'pending').toLowerCase(); return `<span class="status-pill status-${html(st)}">${html(st)}</span>`; }
  function commissionFor(amount){
    amount = Number(amount||0);
    const rules = cfg.COMMISSION_RULES || [];
    const r = rules.find(x => amount >= Number(x.min||0) && amount <= Number(x.max||999999999));
  ...
  async function getProofUrl(path){
    if(!path || !sb) return '';
    if(String(path).startsWith('http')) return path;
    try{ const {data}=await sb.storage.from('payment-proofs').createSignedUrl(path, 1800); return data?.signedUrl || ''; }
    catch{ try{return sb.storage.from('payment-proofs').getPublicUrl(path).data.publicUrl || '';}catch{return '';} }
  }

  async function loadSellerDashboard(){
    const status = q('#sellerStatusChip');
    if(status) status.textContent = sellerActive() ? 'Seller active' : 'Not seller';
    const svc = q('#sellerServiceList'), orders = q('#sellerOrderList'), payouts = q('#sellerPayoutList');
    if(!svc && !orders && !payouts) return;
    if(!sb || !uid()) { [svc,orders,payouts].forEach(el=>{if(el) el.innerHTML='<div class="empty-state">Login to view seller dashboard.</div>';}); return; }
    if(!sellerActive()) { [svc,orders,payouts].forEach(el=>{if(el) el.innerHTML='<div class="empty-state">Apply to sell and get admin approval before using this dashboard.</div>';}); return; }
    const [servicesRes, ordersRes, payoutsRes] = await Promise.all([
      sb.from('seller_services').select('*').eq('seller_id', uid()).order('created_at',{ascending:false}).then(r=>r).catch(e=>({data:[],error:e})),
      sb.from('manual_orders').select('*').eq('seller_id', uid()).order('created_at',{ascending:false}).then(r=>r).catch(e=>({data:[],error:e})),
      sb.from('seller_payouts').select('*').eq('seller_id', uid()).order('created_at',{ascending:false}).then(r=>r).catch(e=>({data:[],error:e}))
    ]);
    const sData = servicesRes.data || [], oData = ordersRes.data || [], pData = payoutsRes.data || [];
    const completed = oData.filter(o=>['approved','completed'].includes(o.status));
    const earned = completed.reduce((a,o)=>a+Number(o.seller_earning_inr||0),0);
    const paid = pData.filter(p=>p.status==='paid').reduce((a,p)=>a+Number(p.amount_inr||0),0);
    q('#sellerTotalEarnings') && (q('#sellerTotalEarnings').textContent = rupee(earned));
    q('#sellerPendingPayout') && (q('#sellerPendingPayout').textContent = rupee(Math.max(0, earned-paid)));
    q('#sellerCompletedOrders') && (q('#sellerCompletedOrders').textContent = completed.length);
    if(svc) svc.innerHTML = sData.map(x=>`<div class="seller-row"><div><b>${html(x.title)}</b><small>${html(x.game||'Game')} · ${html(x.description||'')}</small></div><div>${rupee(x.price_inr)}</div><div>${statusPill(x.status)}</div><div class="actions"><button class="mini danger" onclick="deleteSellerService('${x.id}')">Delete</button></div></div>`).join('') || '<div class="empty-state">No services yet. Create your first listing.</div>';
    if(orders) orders.innerHTML = oData.map(x=>`<div class="seller-row"><div><b>${html(x.item_name||'Order')}</b><small>UTR: ${html(x.utr||'pending')}</small></div><div>${rupee(x.amount_inr)}<small>Earn ${rupee(x.seller_earning_inr)}</small></div><div>${statusPill(x.status)}</div><div class="actions"><small>${new Date(x.created_at||Date.now()).toLocaleDateString()}</small></div></div>`).join('') || '<div class="empty-state">No seller orders yet.</div>';
    if(payouts) payouts.innerHTML = pData.map(x=>`<div class="seller-row"><div><b>${rupee(x.amount_inr)}</b><small>${html(x.note||'Payout request')}</small></div><div>${statusPill(x.status)}</div><div>${x.paid_at ? new Date(x.paid_at).toLocaleDateString() : '—'}</div><div></div></div>`).join('') || '<div class="empty-state">No payout requests yet.</div>';
  }

  async function createService(){
    if(!sb || !uid()) return notify('Login required');
    if(!sellerActive()) return notify('Only approved sellers can create services.');
    const title=q('#serviceTitle')?.value.trim(), game=q('#serviceGame')?.value.trim(), price=Number(q('#servicePrice')?.value||0), description=q('#serviceDescription')?.value.trim();
    if(!title || !price || price < 1) return notify('Enter service title and price.');
    const {error}=await sb.from('seller_services').insert({seller_id:uid(), title, game:game||'Valorant', price_inr:price, description, status:'active'});
    if(error) return notify('Service failed: '+error.message);
    ['#serviceTitle','#serviceGame','#servicePrice','#serviceDescription'].forEach(id=>{const el=q(id); if(el) el.value='';});
    notify('Service created. Buyers can now order it.'); loadSellerDashboard(); loadMarketplaceServices();
  }

  async function loadMarketplaceServices(){
    const el=q('#serviceList'); if(!el || !sb) return;
    const {data,error}=await sb.from('seller_services').select('*').eq('status','active').order('created_at',{ascending:false}).limit(30).then(r=>r).catch(e=>({data:[],error:e}));
    if(error){ el.innerHTML='<div class="empty-state">Services unavailable right now.</div>'; return; }
    if(!data?.length){ el.innerHTML='<div class="empty-state">No real seller services yet. Approved sellers can upload services from Seller Dashboard.</div>'; return; }
    el.innerHTML = data.map(s=>`<article class="card service-card"><span class="chip">${html(s.game||'Gaming')}</span><h3>${html(s.title)}</h3><p>${html(s.description||'Seller service')}</p><div class="card-foot"><b>${rupee(s.price_inr)}</b><button class="btn primary" onclick="orderSellerService('${s.id}', '${html(s.title).replace(/'/g,'&#39;')}', ${Number(s.price_inr||0)}, '${s.seller_id||''}')">Buy</button></div></article>`).join('');
  }

  window.orderSellerService = function(serviceId, name, amount, sellerId){
    if(typeof openManualPayment !== 'function') return notify('Payment modal unavailable');
    openManualPayment({name, amount, item_type:'service', service_id:serviceId, seller_id:sellerId});
  };
  window.deleteSellerService = async function(id){ if(!confirm('Delete this service?')) return; await sb.from('seller_services').delete().eq('id',id); notify('Service deleted'); loadSellerDashboard(); loadMarketplaceServices(); };
  window.requestSellerPayout = async function(){
    if(!sb || !uid()) return notify('Login required');
    const earned = Number((q('#sellerPendingPayout')?.textContent || '0').replace(/[^0-9]/g,''));
    if(!earned || earned < 1) return notify('No pending payout available.');
    const {error}=await sb.from('seller_payouts').insert({seller_id:uid(), amount_inr:earned, status:'pending', note:'Seller requested manual payout'});
    if(error) return notify('Payout request failed: '+error.message);
    notify('Payout requested. Admin will process manually.'); loadSellerDashboard();
  };

  async function loadAdminMoneyCenter(){
    if(!(typeof isAdmin === 'function' && isAdmin()) || !sb) return;
    const orderEl=q('#adminManualOrders'), payEl=q('#adminPayoutRequests');
    if(orderEl){
      const {data}=await sb.from('manual_orders').select('*').order('created_at',{ascending:false}).limit(50).then(r=>r).catch(()=>({data:[]}));
      const rows = await Promise.all((data||[]).map(async o=>{ const img=await getProofUrl(o.proof_url); return `<div class="seller-row"><div><b>${html(o.item_name||'Order')}</b><small>UTR: ${html(o.utr||'')}</small></div><div>${rupee(o.amount_inr)}<small>Seller: ${rupee(o.seller_earning_inr)} · Commission: ${rupee(o.commission_inr)}</small></div><div>${statusPill(o.status)}</div><div class="actions">${img?`<a class="mini" target="_blank" href="${img}">Proof</a>`:''}${o.status==='pending'?`<button class="mini" onclick="adminOrderStatus('${o.id}','approved')">Approve</button><button class="mini danger" onclick="adminOrderStatus('${o.id}','rejected')">Reject</button>`:''}</div></div>`; }));
      orderEl.innerHTML = rows.join('') || 'No manual orders yet.';
    }
    if(payEl){
      const {data}=await sb.from('seller_payouts').select('*').order('created_at',{ascending:false}).limit(50).then(r=>r).catch(()=>({data:[]}));
      payEl.innerHTML = (data||[]).map(p=>`<div class="seller-row"><div><b>${rupee(p.amount_inr)}</b><small>${html(p.seller_id||'seller')}</small></div><div>${statusPill(p.status)}</div><div>${html(p.note||'')}</div><div class="actions">${p.status==='pending'?`<button class="mini" onclick="adminPayoutStatus('${p.id}','paid')">Mark Paid</button><button class="mini danger" onclick="adminPayoutStatus('${p.id}','rejected')">Reject</button>`:''}</div></div>`).join('') || 'No payout requests yet.';
    }
  }
  window.adminOrderStatus = async function(id,status){ await sb.from('manual_orders').update({status, approved_at: status==='approved'?new Date().toISOString():null, rejected_at: status==='rejected'?new Date().toISOString():null}).eq('id',id); notify('Order '+status); loadAdminMoneyCenter(); loadSellerDashboard(); };
  window.adminPayoutStatus = async function(id,status){ await sb.from('seller_payouts').update({status, paid_at: status==='paid'?new Date().toISOString():null}).eq('id',id); notify('Payout '+status); loadAdminMoneyCenter(); };

  const oldSwitch = window.switchView;
  if(typeof oldSwitch === 'function') window.switchView = function(id){ oldSwitch(id); if(id==='sellerDashboard') setTimeout(loadSellerDashboard,80); if(id==='market') setTimeout(loadMarketplaceServices,80); if(id==='admin') setTimeout(loadAdminMoneyCenter,120); };
  const oldRenderAdmin2 = typeof renderAdmin === 'function' ? renderAdmin : null;
  if(oldRenderAdmin2) renderAdmin = async function(){ await oldRenderAdmin2(); await loadAdminMoneyCenter(); };
  document.addEventListener('DOMContentLoaded',()=>{
    q('#createServiceBtn')?.addEventListener('click', createService);
    q('#requestPayoutBtn')?.addEventListener('click', window.requestSellerPayout);
    q('#refreshSellerDashboard')?.addEventListener('click', loadSellerDashboard);
    q('#refreshAdminOrders')?.addEventListener('click', loadAdminMoneyCenter);
    setTimeout(()=>{ loadMarketplaceServices(); loadSellerDashboard(); loadAdminMoneyCenter(); }, 1500);
    try{ if(sb?.channel) sb.channel('seller_money_live').on('postgres_changes',{event:'*',schema:'public',table:'manual_orders'},()=>{loadAdminMoneyCenter();loadSellerDashboard();}).on('postgres_changes',{event:'*',schema:'public',table:'seller_services'},()=>{loadMarketplaceServices();loadSellerDashboard();}).on('postgres_changes',{event:'*',schema:'public',table:'seller_payouts'},()=>{loadAdminMoneyCenter();loadSellerDashboard();}).subscribe(); }catch{}
  });
})();
