...
window.loadSellerDashboard = async() => { if(!$('sellerGate')) return; if(!state.user){ $('sellerGate').innerHTML='<h2>Seller Studio</h2><p>Login to apply as seller and create services.</p><button class="primary" onclick="openAuth()">Login</button>'; return; } const status=canSell()?'approved':(state.profile?.seller_status||'none'); $('sellerGate').innerHTML=`<div class="section-head"><div><h2>Seller status: ${escapeHtml(status)}</h2><p>${canSell()?'You can create live service listings.':'Apply and upload proof later if needed. Admin reviews manually.'}</p></div><button class="primary" onclick="applySeller()">${status==='pending'?'Application pending':'Apply to Sell'}</button></div>`; await Promise.all([loadMyServices(),loadSellerOrders(),loadSellerWallet()]); };
...
function renderIdentity(){ if(!state.profile) return; $('identityStrip').innerHTML=`<div><span class="tag">${state.isAdmin?'Founder':'Player'}</span><b>${Number(state.profile.xp||0).toLocaleString('en-IN')} XP</b></div><div><b>${escapeHtml(state.profile.equipped_title||'Rookie')}</b><span>Title</span></div><div><b>${escapeHtml(state.profile.equipped_badge||'First Login')}</b><span>Badge</span></div><div><b>${escapeHtml(state.profile.equipped_banner||'Starter Arena')}</b><span>Banner</span></div>`; const eq=$('equippedIdentity'); if(eq) eq.innerHTML=`<article class="item-card"><b>${escapeHtml(state.profile.equipped_title||'Rookie')}</b><p>Title</p></article><article class="item-card"><b>${escapeHtml(state.profile.equipped_badge||'First Login')}</b><p>Badge</p></article><article class="item-card"><b>${escapeHtml(state.profile.equipped_banner||'Starter Arena')}</b><p>Banner</p></article>`; }
function renderRewards(){ $('rewardsGrid').innerHTML=state.rewards.map(r=>{ const unlocked=state.isAdmin||state.unlocked.has(r.key); const lockedAdmin=r.admin_only&&!state.isAdmin; return `<article class="reward-card ${r.admin_only?'admin-only-card':''}"><span class="reward-type">${r.admin_only?'1 of 1 admin':r.type}</span><h3>${escapeHtml(r.name)}</h3><p>${escapeHtml(r.description||'')}</p><b>${r.admin_only?'Founder only':(Number(r.cost||0)+' XP')}</b><button class="${unlocked?'ghost':'primary'}" onclick="${unlocked?`equipReward('${r.key}')`:`unlockReward('${r.key}')`}" ${lockedAdmin?'disabled':''}>${unlocked?'Equip':'Unlock'}</button></article>`; }).join(''); }
...
async function loadLeaderboard(){ let data=[]; if(supabase){ const res=await supabase.from('profiles').select('username,display_name,xp,equipped_title').order('xp',{ascending:false}).limit(10); data=res.data||[]; } else data=[state.profile].filter(Boolean); $('leaderboard').innerHTML=data.length?data.map((p,i)=>`<article class="item-card"><b>#${i+1} ${escapeHtml(p.display_name||p.username||'player')}</b><p>${Number(p.xp||0).toLocaleString('en-IN')} XP • ${escapeHtml(p.equipped_title||'Rookie')}</p></article>`).join(''):'No leaderboard yet.'; }
...
async function loadAdminStats(){ if(!supabase) return; $('admUsers').textContent=(await supabase.from('profiles').select('id',{count:'exact',head:true})).count||0; $('admOrders').textContent=(await supabase.from('manual_orders').select('id',{count:'exact',head:true}).eq('status','pending')).count||0; $('admSellerApps').textContent=(await supabase.from('seller_applications').select('id',{count:'exact',head:true}).eq('status','pending')).count||0; const g=await supabase.from('manual_orders').select('amount').eq('status','approved'); $('admGMV').textContent=money((g.data||[]).reduce((a,o)=>a+Number(o.amount||0),0)); }
